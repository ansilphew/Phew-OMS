...
export default function CEOProposalManagementPage() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    // If fallback mock item, update in local state
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-4 pt-2">
              ...
              <button
                type="button"
                onClick={handleClear}
                className="px-8 py-3 rounded-xl border border-[#cbd5e1] bg-white text-sm font-semibold text-[#4a5568] hover:bg-slate-50 hover:text-slate-800 transition cursor-pointer outline-none shadow-xs"
              >
                    ...
                    <tr key={prop._id} className="hover:bg-[#fafafa] transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-3.5">
                          <button
                            onClick={() => router.push(`/ceo/proposal-management/${prop._id}`)}
